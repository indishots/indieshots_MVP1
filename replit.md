...
Recent Changes:
- July 3, 2025: **PROMO CODE TIER SYNCHRONIZATION COMPLETELY FIXED** - Resolved final issue where users signing up with Gmail + INDIE2025 promo code weren't getting proper pro tier access; implemented quota manager synchronization in hybrid signup controller, removed duplicate /api/auth/user endpoints, fixed frontend navigation issues in verification page, and ensured all tier management systems work in harmony; users now automatically receive unlimited pages, unlimited shots, storyboard generation, and no upgrade prompts when using valid promo codes
