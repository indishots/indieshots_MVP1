...
Recent Changes:
- July 3, 2025: **PROMO CODE TIER UPGRADE SYSTEM COMPLETELY FIXED** - Resolved systematic issue where promo code users weren't getting pro tier benefits during signup; fixed race condition between user creation and promo code application by reordering logic to create user with correct tier first; modified PromoCodeService to avoid unnecessary tier overwrites; fixed usage data preservation for returning users who delete Firebase but keep PostgreSQL records; all promo code signups now properly receive tier='pro' with unlimited access
