...
Recent Changes:
- July 4, 2025: **PROMO CODE TIER ASSIGNMENT COMPLETELY FIXED** - Resolved critical issue where users signing up with INDIE2025 promo code weren't receiving pro tier access properly; enhanced verification page to show tier-specific success messages (Pro Account Activated vs Account Created), added session storage for promo code persistence through verification flow, improved backend sync logging to confirm tier assignment, and added 1-second delay before dashboard redirect to ensure authentication state propagation; users with INDIE2025 now immediately receive pro tier with unlimited pages, unlimited shots, and storyboard generation capabilities
