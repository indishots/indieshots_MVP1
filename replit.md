...
Recent Changes:
- July 3, 2025: **AUTOMATIC TIER VALIDATION SYSTEM IMPLEMENTED** - Fixed critical promo code signup issue where users weren't immediately getting pro tier access after verification; implemented automatic tier validation middleware that checks and updates user tier on every authenticated request; added immediate PostgreSQL sync during OTP verification with JWT token generation; created useTierValidation hook for frontend that automatically refreshes user data 2 seconds after login; users now get instant pro tier access with INDIE2025 promo code without manual page refresh
