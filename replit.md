...
Recent Changes:
- July 4, 2025: **PROMO CODE TIER ASSIGNMENT SYSTEM COMPLETELY FIXED** - Resolved critical issue where users signing up with INDIE2025 promo code weren't receiving pro tier access; fixed dhulipallagopichandu@gmail.com account that was incorrectly assigned free tier instead of pro tier; implemented comprehensive fix including enhanced promo code application logging in hybrid verification controller with INDIE2025 specific validation, improved Firebase sync controller with pro tier correction logic, database corrections for affected accounts, and robust tier synchronization; users with INDIE2025 now consistently receive pro tier (unlimited pages: -1, unlimited shots: -1, storyboard generation: true) while users without promo codes receive free tier (5 pages, 5 shots, no storyboards)
