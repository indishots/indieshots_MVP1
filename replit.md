...
Recent Changes:
- July 4, 2025: **DYNAMIC PROMO CODE SYSTEM FULLY IMPLEMENTED** - Replaced hardcoded account protection with universal dynamic promo code validation that works for ALL existing and future users; implemented comprehensive dynamic tier detection in storage layer using proper Drizzle ORM queries to check promo_code_usage table; updated all frontend components (authentication manager, header, left panel, dashboard, upgrade status) to rely on backend tier data instead of hardcoded lists; fixed promo code usage records to include granted_tier values for proper validation; system now works universally: ANY user with active promo code (INDIE2025 or future codes) automatically receives granted tier on every login/data access, users without promo codes receive standard free tier; eliminates need for hardcoded account maintenance while ensuring consistent tier assignment across all authentication flows
