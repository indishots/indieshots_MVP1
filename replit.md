...
Recent Changes:
- July 4, 2025: **INDIE2025 UNIVERSAL PROMO CODE SYSTEM IMPLEMENTED** - Completely fixed promo code tier assignment for all users; both gopichandudhulipalla@gmail.com and dhulipallagopichandu@gmail.com now have pro tier access with INDIE2025 promo code usage recorded; implemented universal validation system in Firebase sync controller that automatically detects and fixes any user with INDIE2025 promo code but incorrect tier; enhanced hybrid verification controller with comprehensive INDIE2025 specific logging and failsafe mechanisms; created PromoCodeUniversalValidator utility for ongoing tier validation; system now guarantees that ANY user with INDIE2025 promo code receives pro tier (unlimited pages: -1, unlimited shots: -1, storyboard generation: true) while users without promo codes receive free tier (5 pages, 5 shots, no storyboards); includes automatic correction on every signin to ensure tier consistency
